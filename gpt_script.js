...
async function getBotResponse(userMessage) {
    const apiKey = "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"; // Replace with your actual API key
    const apiUrl = "https://api.groq.com/openai/v1/chat/completions";

    // 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
    const requestBody = {
...
const side_bar = document.querySelector(".settings-icon");
side_bar.addEventListener("click", function(event) {
    const sidish = document.querySelector(".sidebar");
    sidish.style.visibility = "visible";
})

const close = document.querySelector(".CloseTab");
close.addEventListener("click", function(event) {
    const sidish = document.querySelector(".sidebar");
    sidish.style.visibility = "hidden";
})
